...
class LobiAPI:
	...
	def GetPokesAll(self, group_id, chat_id):
		...
		data = { "id": chat_id }
		while True:
			res = self.GET(2, "group/{group_id}/chats/pokes".format(group_id=group_id), data)
			if res == None or res.get("users") == None or len(res["users"]) == 0:
				break
			result.extend(res["users"])
			if res.get("next_cursor") == None or res["next_cursor"] == -1 or res["next_cursor"] == 0:
	...
	def POST(self, version, request_url, query = {}):
        ...
        def PostMessage(self,group_id,message):
                url = "https://api.lobi.co/1/group/{group_id}/chats".format(group_id=group_id)
		data = {
			"lang": "en",
                        "message": message,
			"token": self.Token,
        ...
        def PostReply(self,group_id,chat_id,message):
                url = "https://api.lobi.co/1/group/{group_id}/chats".format(group_id=group_id)
		data = {
			"lang": "en",
                        "message": message,
		...
		response = requests.post(url,data)
                return response.json()
        def MakePrivateGroup(self,user_id):
                url = "https://web.lobi.co/api/groups/1on1s"
		data = {
                        "user": user_id
		}
                headers = {
                        ...
                        "Cookie":"_ga=GA1.2.1867834487.1592919202; lnubid=bqL35a1592919210.41835; installId=30122f5ea537671d46eed11047539cc7c87b7b46; __gads=ID=18cea69ad2ac2060:T=1607353223:S=ALNI_MZGI39eC4plNOuVpLYscdewr15-Pw; _gid=GA1.2.2066270152.1609998250; is_signed_in=1; nsess=535496626bacf52b2ba1b1b358cb51c620189c6d; lastReadNotificationId=9526122117; _gat=1"
                }
